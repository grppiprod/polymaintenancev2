...
// --- DATA MAPPERS ---
const mapUserFromDB = (u: any): User => ({ id: u.id, username: u.username, password: u.password, fullName: u.full_name, role: u.role as Role });
const mapLogFromDB = (l: any): MaintenanceLog => ({
  id: l.id, title: l.title, description: l.description, type: l.type as LogType, priority: l.priority as Priority,
  status: l.status as LogStatus, imageUrl: l.image_url, createdBy: l.created_by, creatorName: l.creator_name,
  creatorRole: l.creator_role as Role, createdAt: l.created_at, closedAt: l.closed_at, history: l.history || []
});
const mapUserToDB = (u: User) => ({ id: u.id, username: u.username, password: u.password, full_name: u.fullName, role: u.role });
const mapLogToDB = (l: MaintenanceLog) => ({
  id: l.id, title: l.title, description: l.description, type: l.type, priority: l.priority, status: l.status,
  image_url: l.imageUrl, created_by: l.createdBy, creator_name: l.creatorName, creator_role: l.creatorRole,
  created_at: l.createdAt, closed_at: l.closedAt, history: l.history
});

interface ToastMessage { id: string; message: string; type: 'info' | 'success' | 'error'; }
...
create table if not exists users ( id text primary key, username text not null, password text not null, full_name text not null, role text not null );
create table if not exists logs ( id text primary key, title text not null, description text not null, type text not null, priority text not null, status text not null, image_url text, created_by text not null, creator_name text not null, creator_role text not null, created_at timestamptz not null, closed_at timestamptz, history jsonb default '[]'::jsonb );
create table if not exists user_log_views ( user_id text not null, log_id text not null, last_viewed_at timestamptz not null, primary key (user_id, log_id) );
alter publication supabase_realtime add table logs; alter publication supabase_realtime add table users; alter publication supabase_realtime add table user_log_views;`;
  const handleCopy = () => { navigator.clipboard.writeText(sqlSetup); setCopied(true); setTimeout(() => setCopied(false), 2000); };

  return (
    <div className="min-h-screen bg-dark-950 flex flex-col items-center justify-center p-4">
      <div className="max-w-2xl w-full space-y-6">
        <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-6 text-center">
          <div className="bg-red-500/20 w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-4"><AlertTriangle className="text-red-500" size={24} /></div>
          <h2 className="text-xl font-bold text-white mb-2">Database Connection Error</h2>
          <p className="text-red-300 font-mono text-sm bg-dark-950 p-2 rounded border border-red-500/20 inline-block">{errorMessage}</p>
        </div>
        <div className="bg-dark-900 border border-dark-800 rounded-xl overflow-hidden">
          <div className="p-4 border-b border-dark-800 bg-dark-950 flex justify-between items-center">
            <div className="flex items-center gap-2 text-zinc-400"><Database size={18} /><span className="font-medium text-sm">Required Database Setup</span></div>
            <button onClick={handleCopy} className="flex items-center gap-2 text-xs font-medium bg-brand-500/10 text-brand-500 hover:bg-brand-500/20 px-3 py-1.5 rounded transition-colors">{copied ? <CheckCircle size={14} /> : <Copy size={14} />}{copied ? 'Copied SQL' : 'Copy SQL'}</button>
          </div>
          <div className="p-4 bg-black/50 overflow-x-auto"><pre className="text-xs text-zinc-300 font-mono whitespace-pre-wrap">{sqlSetup}</pre></div>
        </div>
        <button onClick={onRetry} className="w-full bg-white text-black hover:bg-zinc-200 font-bold py-3 rounded-xl transition-colors flex items-center justify-center gap-2"><RefreshCw size={20} /> Retry Connection</button>
      </div>
    </div>
  );
};
...
const Login = ({ onLogin, users, isLoading }: { onLogin: (u: User) => void, users: User[], isLoading: boolean }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (users.length === 0 && username === 'admin' && password === '1234') { onLogin({ id: 'temp_admin', username: 'admin', fullName: 'System Admin', role: Role.ADMIN }); return; }
  ...
  return (
    <div className="min-h-screen bg-dark-950 flex items-center justify-center p-4">
      <div className="bg-dark-900 border border-dark-800 p-8 rounded-2xl w-full max-w-md shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col items-center mb-8"><div className="bg-brand-500/10 p-4 rounded-full mb-4"><Wrench className="w-10 h-10 text-brand-500" /></div><h1 className="text-3xl font-bold text-white tracking-tight">PolyMaintenance</h1><p className="text-zinc-400 mt-2 text-center">Enter your credentials.</p></div>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div><label className="block text-sm font-medium text-zinc-400 mb-2">Username</label><input type="text" value={username} onChange={(e) => setUsername(e.target.value)} className="w-full bg-dark-950 border border-dark-700 rounded-lg p-3 text-white focus:ring-2 focus:ring-brand-500 outline-none" placeholder="e.g. admin" disabled={isLoading} /></div>
          <div><label className="block text-sm font-medium text-zinc-400 mb-2">Password</label><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-dark-950 border border-dark-700 rounded-lg p-3 text-white focus:ring-2 focus:ring-brand-500 outline-none" placeholder="••••••••" disabled={isLoading} /></div>
          {error && <p className="text-red-400 text-sm text-center animate-pulse">{error}</p>}
          <button type="submit" disabled={isLoading} className="w-full bg-brand-500 hover:bg-brand-600 disabled:opacity-50 text-white font-semibold p-3 rounded-lg transition-colors shadow-lg shadow-brand-500/20 flex items-center justify-center gap-2">{isLoading ? <Loader2 className="animate-spin" size={20} /> : 'Sign In'}</button>
        </form>
      </div>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm no-print">
      <div className={`bg-dark-900 rounded-xl border border-dark-800 w-full ${sizeClasses[size as keyof typeof sizeClasses]} max-h-[90vh] overflow-hidden flex flex-col shadow-2xl animate-fade-in-up`}>
        <div className="flex justify-between items-center p-4 md:p-6 border-b border-dark-800"><h2 className="text-lg md:text-xl font-bold text-white line-clamp-1">{title}</h2><button onClick={onClose} className="text-zinc-400 hover:text-white transition-colors"><X size={24} /></button></div>
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={`Create New ${activeTab === LogType.REPAIR ? 'Repair' : 'Maintenance'} Log`}>
            <form onSubmit={handleSubmit} className="space-y-4">
                <div><label className="block text-sm font-medium text-zinc-400 mb-2">Attach Picture</label><div className="flex items-center gap-4"><div className="w-20 h-20 bg-dark-950 border border-dashed border-dark-700 rounded-lg flex items-center justify-center overflow-hidden shrink-0">{form.image ? <img src={URL.createObjectURL(form.image)} className="w-full h-full object-cover" /> : <span className="text-[10px] text-zinc-600">No Image</span>}</div><label className="cursor-pointer bg-dark-800 hover:bg-dark-700 border border-dark-700 text-zinc-300 px-3 py-2 rounded-lg text-sm flex items-center gap-2 transition-colors"><FileImage size={16} /> Upload Image<input type="file" accept="image/*" className="hidden" onChange={(e) => setForm({...form, image: e.target.files ? e.target.files[0] : null})} /></label></div></div>
                ...
                <div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-medium text-zinc-400 mb-1">Priority</label><select value={form.priority} onChange={e => setForm({...form, priority: e.target.value as Priority})} className="w-full bg-dark-950 border border-dark-700 rounded-lg p-2.5 text-white text-sm">{Object.values(Priority).map(p => <option key={p} value={p}>{p}</option>)}</select></div></div>
                <div className="flex justify-end pt-4 gap-2"><button type="button" onClick={onClose} className="px-4 py-2 text-sm text-zinc-400 hover:text-white transition-colors">Cancel</button><button type="submit" disabled={isLoading} className="bg-brand-500 hover:bg-brand-600 disabled:opacity-50 text-white px-6 py-2 rounded-lg text-sm font-medium flex items-center gap-2">{isLoading && <Loader2 className="animate-spin" size={16} />} Create Log</button></div>
            </form>
        </Modal>
    );
};
...
const ChangePasswordModal = ({ isOpen, onClose, onSubmit, isLoading }: any) => {
    ...
    const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); if (form.new !== form.confirm) { setError("Passwords do not match."); return; } onSubmit(form.current, form.new, (err: string) => setError(err)); };
    return (
...
const LogDetailModal = ({ log, currentUser, isOpen, onClose, onAddHistory, onCloseLog, onDeleteLog, onDeleteHistory }: any) => {
  ...
  if (!log) return null;
  const handleAddNote = async () => { setIsSubmitting(true); await onAddHistory(log.id, note); setNote(''); setIsSubmitting(false); }
  ...
  return (
    <>
      <Modal isOpen={isOpen} onClose={onClose} title="Log Details" size="lg">
        <div className="space-y-4 md:space-y-6">
          <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
            <div><h1 className="text-xl md:text-2xl font-bold text-white mb-2">{log.title}</h1><div className="flex flex-wrap gap-2 mb-2 md:mb-4"><span className={`px-2 py-0.5 text-xs font-semibold rounded border ${PRIORITY_COLORS[log.priority as Priority]}`}>{log.priority}</span><span className={`px-2 py-0.5 text-xs font-semibold rounded border ${log.status === LogStatus.ACTIVE ? 'bg-brand-500/10 text-brand-400 border-brand-500/20' : 'bg-zinc-700 text-zinc-300 border-zinc-600'}`}>{log.status}</span><span className={`px-2 py-0.5 text-xs font-semibold rounded border ${ROLE_BADGES[log.creatorRole as Role]}`}>{log.creatorRole}</span></div></div>
            {log.status === LogStatus.ACTIVE && (<button onClick={() => onCloseLog(log.id)} className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 justify-center"><CheckCircle size={16} /> Mark as Closed</button>)}
          </div>
          {log.imageUrl && (<div className="rounded-xl overflow-hidden border border-dark-700 bg-black/50 aspect-video flex items-center justify-center relative group"><img src={log.imageUrl} alt="Issue" className="w-full h-full object-contain" /><a href={log.imageUrl} target="_blank" rel="noreferrer" className="absolute bottom-4 right-4 bg-black/80 text-white px-3 py-1 rounded text-xs opacity-0 group-hover:opacity-100 transition-opacity">View Full</a></div>)}
          <div className="bg-dark-950 p-3 md:p-4 rounded-lg border border-dark-800"><h3 className="text-sm font-medium text-zinc-400 mb-1">Description</h3><p className="text-zinc-100 whitespace-pre-wrap text-sm md:text-base">{log.description}</p><div className="mt-4 flex items-center gap-2 text-xs text-zinc-500"><span className="font-medium text-zinc-400">{log.creatorName}</span><span>•</span><span>{formatDate(log.createdAt)}</span></div></div>
          {canDeleteLog && (<div className="flex justify-end"><button onClick={() => { if(confirm('Delete log?')) { onDeleteLog(log.id); onClose(); } }} className="text-red-400 hover:text-red-300 text-sm flex items-center gap-1"><Trash2 size={14} /> Delete Log</button></div>)}
          ...
          <div>
            ...
            <div className="space-y-4 mb-6 relative pl-4 border-l-2 border-dark-800">
               {log.history.sort((a: HistoryItem, b: HistoryItem) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()).map((item: HistoryItem) => (
                 <div key={item.id} className="relative pl-6 pb-2"><div className="absolute -left-[25px] top-1 w-4 h-4 rounded-full bg-dark-950 border-2 border-brand-500 box-content"></div><div className="bg-dark-800 p-3 rounded-lg border border-dark-700 hover:border-dark-600 transition-colors group"><div className="flex justify-between items-start mb-1"><div className="flex items-center gap-2"><span className="font-medium text-white text-xs md:text-sm">{item.creatorName}</span><span className={`text-[10px] px-1.5 rounded border ${ROLE_BADGES[item.creatorRole]}`}>{item.creatorRole}</span></div><span className="text-[10px] md:text-xs text-zinc-500">{formatDate(item.createdAt)}</span></div><p className="text-zinc-300 text-sm">{item.content}</p>{isCreatorOrAdmin(item.createdBy) && (<button onClick={() => onDeleteHistory(log.id, item.id)} className="mt-2 text-xs text-red-500 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity">Delete</button>)}</div></div>
               ))}
               {log.history.length === 0 && <p className="text-zinc-500 text-sm italic pl-6">No history recorded yet.</p>}
            </div>
            {log.status === LogStatus.ACTIVE && (<div className="flex gap-2 items-start bg-dark-950 p-4 rounded-lg border border-dark-800"><textarea value={note} onChange={(e) => setNote(e.target.value)} placeholder="Add a progress update..." className="flex-1 bg-dark-900 border border-dark-700 rounded-lg p-2 text-white text-sm focus:ring-1 focus:ring-brand-500 outline-none resize-none h-20"></textarea><button disabled={!note.trim() || isSubmitting} onClick={handleAddNote} className="bg-brand-500 hover:bg-brand-600 disabled:opacity-50 disabled:cursor-not-allowed text-white p-2 rounded-lg">{isSubmitting ? <Loader2 className="animate-spin" size={20} /> : <Save size={20} />}</button></div>)}
          </div>
      ...
      <div id="print-area" className="print-only hidden bg-white text-black p-6 font-serif max-w-[210mm] mx-auto"><h1 className="text-2xl font-bold mb-4">{log.title}</h1><p>{log.description}</p></div>
    </>
  );
};
...
const App = () => {
  // State
  const [currentUser, setCurrentUser] = useState<User | null>(() => { try { const saved = localStorage.getItem('polymaintenance_user'); return saved ? JSON.parse(saved) : null; } catch (e) { return null; } });
  ...
  const markLogAsRead = async (logId: string) => {
      const now = new Date().toISOString(); setLastViewedLogs(prev => ({ ...prev, [logId]: now }));
      if (!currentUser) return; try { await supabase.from('user_log_views').upsert({ user_id: currentUser.id, log_id: logId, last_viewed_at: now }); } catch (e) {}
  };

  const handleOpenLogCard = (log: MaintenanceLog) => { if (isSelectionMode) { toggleLogSelection(log.id); } else { setSelectedLog(log); markLogAsRead(log.id); } };
  ...
  const handleCreateLog = async (formData: any) => {
    if (!currentUser) return; setIsActionLoading(true); let imageUrl = ''; if (formData.image) { try { imageUrl = await compressImage(formData.image); } catch (err) { alert("Image error"); setIsActionLoading(false); return; } }
    const now = new Date().toISOString(); const newLog: MaintenanceLog = { id: generateId(), title: formData.title, description: formData.description, priority: formData.priority, status: LogStatus.ACTIVE, type: activeTab, createdBy: currentUser.id, creatorName: currentUser.fullName, creatorRole: currentUser.role, createdAt: now, imageUrl: imageUrl, history: [] };
    try { const { error } = await supabase.from('logs').insert([mapLogToDB(newLog)]); if (error) throw error; setLogs(prev => [newLog, ...prev]); await supabase.from('user_log_views').upsert({ user_id: currentUser.id, log_id: newLog.id, last_viewed_at: now }); setLastViewedLogs(prev => ({ ...prev, [newLog.id]: now })); setIsCreateModalOpen(false); addToast('Log created', 'success'); } catch (error: any) { alert("Failed to save log."); } finally { setIsActionLoading(false); }
  };
  const handleAddHistory = async (logId: string, content: string) => {
    if (!currentUser) return; const currentLog = logs.find(l => l.id === logId); if (!currentLog) return; const newHistory: HistoryItem = { id: generateId(), content, createdBy: currentUser.id, creatorName: currentUser.fullName, creatorRole: currentUser.role, createdAt: new Date().toISOString() }; const updatedHistory = [...currentLog.history, newHistory];
    try { const { error } = await supabase.from('logs').update({ history: updatedHistory }).eq('id', logId); if (error) throw error; setLogs(prev => prev.map(log => log.id === logId ? { ...log, history: updatedHistory } : log)); markLogAsRead(logId); if (selectedLog?.id === logId) setSelectedLog({ ...selectedLog, history: updatedHistory }); addToast('Note added', 'success'); } catch (error: any) { alert("Failed to update."); }
  };
  const handleCloseLog = async (logId: string) => {
    if(!currentUser) return; const currentLog = logs.find(l => l.id === logId); if (!currentLog) return; const closeNote: HistoryItem = { id: generateId(), content: 'Log marked as CLOSED.', createdBy: currentUser.id, creatorName: currentUser.fullName, creatorRole: currentUser.role, createdAt: new Date().toISOString() }; const updatedHistory = [...currentLog.history, closeNote]; const closedAt = new Date().toISOString();
    try { const { error } = await supabase.from('logs').update({ status: LogStatus.CLOSED, closed_at: closedAt, history: updatedHistory }).eq('id', logId); if (error) throw error; setLogs(prev => prev.map(log => log.id === logId ? { ...log, status: LogStatus.CLOSED, closedAt, history: updatedHistory } : log)); markLogAsRead(logId); addToast('Log closed', 'success'); } catch (error: any) { alert("Failed to close."); }
  };
  const handleDeleteLog = async (logId: string) => { try { const { error } = await supabase.from('logs').delete().eq('id', logId); if (error) throw error; setLogs(prev => prev.filter(l => l.id !== logId)); if (selectedLog?.id === logId) setSelectedLog(null); addToast('Log deleted', 'success'); } catch (error: any) { alert("Failed to delete."); } };
  const handleBulkDelete = async () => { if (!currentUser || currentUser.role !== Role.ADMIN || selectedLogIds.size === 0) return; if(!confirm(`Delete ${selectedLogIds.size} logs?`)) return; setIsActionLoading(true); try { const idsToDelete = Array.from(selectedLogIds); const { error } = await supabase.from('logs').delete().in('id', idsToDelete); if (error) throw error; setLogs(prev => prev.filter(l => !selectedLogIds.has(l.id))); setSelectedLogIds(new Set()); setIsSelectionMode(false); addToast(`${idsToDelete.length} logs deleted`, 'success'); } catch (error: any) { alert("Failed to delete."); } finally { setIsActionLoading(false); } };
  const handleDeleteHistory = async (logId: string, historyId: string) => { const currentLog = logs.find(l => l.id === logId); if (!currentLog) return; const updatedHistory = currentLog.history.filter(h => h.id !== historyId); try { const { error } = await supabase.from('logs').update({ history: updatedHistory }).eq('id', logId); if (error) throw error; setLogs(prev => prev.map(log => log.id === logId ? { ...log, history: updatedHistory} : log)); if(selectedLog) setSelectedLog({ ...selectedLog, history: updatedHistory }); addToast('History item removed', 'success'); } catch (error: any) { alert("Failed."); } };
  const handleCreateUser = async (formData: any) => { setIsActionLoading(true); const newUser: User = { id: generateId(), username: formData.username, password: 'password', fullName: formData.fullName, role: formData.role }; try { const { error } = await supabase.from('users').insert([mapUserToDB(newUser)]); if (error) throw error; setUsers(prev => [...prev, newUser]); setIsUserModalOpen(false); addToast('User created', 'success'); } catch (error: any) { alert("Failed."); } finally { setIsActionLoading(false); } };
  const handleChangePassword = async (current: string, newPass: string, onError: (msg: string) => void) => { if (!currentUser) return; setIsActionLoading(true); if (currentUser.password !== current) { onError("Incorrect current password."); setIsActionLoading(false); return; } try { const { error } = await supabase.from('users').update({ password: newPass }).eq('id', currentUser.id); if (error) throw error; const updatedUser = { ...currentUser, password: newPass }; setCurrentUser(updatedUser); localStorage.setItem('polymaintenance_user', JSON.stringify(updatedUser)); setUsers(prev => prev.map(u => u.id === currentUser.id ? updatedUser : u)); setIsChangePasswordOpen(false); addToast('Password updated', 'success'); } catch (error: any) { onError("Failed."); } finally { setIsActionLoading(false); } };
  const handleDeleteUser = async (userId: string) => { if(userId === currentUser?.id || !confirm("Are you sure?")) return; try { const { error } = await supabase.from('users').delete().eq('id', userId); if (error) throw error; setUsers(prev => prev.filter(u => u.id !== userId)); addToast('User deleted', 'success'); } catch (error: any) { alert("Failed."); } };
  const toggleSelectionMode = () => { setIsSelectionMode(!isSelectionMode); setSelectedLogIds(new Set()); };
  const toggleLogSelection = (logId: string) => { const newSet = new Set(selectedLogIds); if (newSet.has(logId)) newSet.delete(logId); else newSet.add(logId); setSelectedLogIds(newSet); };
  
  const unreadCounts = { [LogType.REPAIR]: logs.filter(l => l.type === LogType.REPAIR && getLogNotificationStatus(l) !== 'read').length, [LogType.PREVENTIVE_MAINTENANCE]: logs.filter(l => l.type === LogType.PREVENTIVE_MAINTENANCE && getLogNotificationStatus(l) !== 'read').length, active: logs.filter(l => l.type === activeTab && l.status === LogStatus.ACTIVE && getLogNotificationStatus(l) !== 'read').length };
  ...
  filteredLogs.sort((a, b) => { if (sortBy === 'priority') { const weightA = PRIORITY_WEIGHTS[a.priority]; const weightB = PRIORITY_WEIGHTS[b.priority]; if (weightA !== weightB) return weightB - weightA; return new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime(); } else if (sortBy === 'date_oldest') { return new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime(); } else { return new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime(); } });
  ...
  return (
    <div className="flex h-screen bg-dark-950 text-zinc-100 overflow-hidden relative">
      <div className="fixed top-4 right-4 z-[60] flex flex-col gap-2 no-print w-full max-w-xs pointer-events-none">
        {toasts.map(toast => ( <div key={toast.id} className={`pointer-events-auto p-4 rounded-lg shadow-xl border border-white/10 text-white text-sm font-medium animate-in slide-in-from-right fade-in flex items-start gap-3 backdrop-blur-md ${toast.type === 'success' ? 'bg-emerald-600/90' : toast.type === 'error' ? 'bg-red-600/90' : 'bg-brand-600/90'}`}><div className="mt-0.5 shrink-0">{toast.type === 'success' ? <CheckCircle size={16} /> : toast.type === 'error' ? <AlertTriangle size={16} /> : <Info size={16} />}</div><div className="flex-1 leading-snug">{toast.message}</div><button onClick={() => setToasts(prev => prev.filter(t => t.id !== toast.id))} className="opacity-70 hover:opacity-100"><X size={14}/></button></div> ))}
      </div>
